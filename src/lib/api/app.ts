...
app.use('*', async (c, next) => {
  ...
  // HSTS and CSP only in production
  if (process.env.NODE_ENV === 'production') {
    c.header('Strict-Transport-Security', 'max-age=31536000; includeSubDomains');
    c.header(
      'Content-Security-Policy',
      "default-src 'self'; script-src 'self' 'unsafe-inline' 'unsafe-eval'; style-src 'self' 'unsafe-inline'; img-src 'self' data: https:; connect-src 'self' https://*.supabase.co https://*.helius-rpc.com https://api.anthropic.com https://cloud.langfuse.com https://*.upstash.io; font-src 'self' data:; frame-ancestors 'none'"
    );
  }
});
...
app.route('/social/oauth', socialOAuthRoutes);
